...
// grammars at runtime, and Sparrowhawk only needs the small GrmManager surface
// below: load named StdArc FSTs from an STTable FAR, look them up, and compose
// a rule with an input FST. This avoids linking the full Thrax runtime while
// preserving the FAR artifact format.
#pragma once
...
class GrmManager {
   public:
    ...
    bool Rewrite(
        const std::string& rule, const Transducer& input,
        MutableTransducer* output, const std::string& pdt_parens_rule = "") const {
        if (output == nullptr)
            return false;
