...
- Changed volatility from `STABLE` to `VOLATILE`.
- Added `SECURITY DEFINER` and `set search_path = public, auth` to ensure permissions.
...
1. **Silent Hook Failure (The "Stable" Bug)**
   ...
   - **Root Cause:** The function was defined as `STABLE` (promising no side effects), but contained an `UPDATE` statement for the monthly reset logic. Postgres blocked the write, causing an error that was swallowed or resulted in early exit.
