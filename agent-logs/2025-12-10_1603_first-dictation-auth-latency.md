...
- ✅ **DRY Refactoring** - Extracted duplicated retry logic into reusable `retryWithBackoff()` utility function
...
**Files Modified:**
- `src/utils/retryWithBackoff.ts` (NEW) - Reusable retry utility with exponential backoff, exported constants for configuration
- `src/components/App.tsx` (lines 590-593, 647-650) - Replaced duplicated retry logic with `retryWithBackoff()` calls for both app startup and sign-in
- `src/hooks/useTranscription.ts` (lines 1248-1254, 2281-2286, 665-672) - Added connection state logging, cleaned up preConnect error handling, suppressed code 1000 warnings

**Key Code Pattern:**
```typescript
// Clean usage with extracted utility (handles Sentry errors: code 1003, 1006, auth timeout)
retryWithBackoff(
  () => trans.preConnect(),
  { context: 'Pre-connect on startup' }
);

// Utility implementation (src/utils/retryWithBackoff.ts)
...
export async function retryWithBackoff<T>(
  ...
  const { maxRetries = 3, initialDelayMs = 500, backoffMultiplier = 2, context } = options;
  let retries = 0;
  ...
  while (retries < maxRetries) {
    try {
      ...
      if (retries < maxRetries) {
        ...
        console.warn(`[App] ${context} attempt ${retries} failed, retrying in ${backoffMs}ms`);
        await new Promise(resolve => setTimeout(resolve, backoffMs));
      } else {
        console.warn(`[App] ${context} failed after ${maxRetries} attempts`);
      }
    }
  }
}
...
5. **Code Duplication (DRY Violation)** - Initial implementation had 23-line retry logic duplicated in two places (startup and sign-in)
   - **Issue:** Maintenance burden, magic numbers hardcoded twice, violates DRY principles
   - **Fix:** Extracted to `retryWithBackoff()` utility function with exported constants, reduced each call site from 23 lines to 3 lines
...
- **Refactor After Validation** - Implementing the fix first (even with duplication) and then refactoring to DRY was the right approach. This let us validate the behavior works before optimizing the code structure. The two-step process (fix → refactor) is safer than trying to be clever upfront.
...
- **DRY Refactoring Post-Implementation** - After validating the fix worked, extracted duplicated retry logic (23 lines × 2 = 46 lines) into a reusable utility function (3 lines per call site). This makes the code more maintainable and the retry configuration easy to tune in one place. The utility is generic enough to be reused for other network-dependent operations (e.g., pre-loading user settings, pre-fetching permissions state).
...
- ✅ **Reusable Retry Utility** - `retryWithBackoff()` available for other network-dependent operations throughout the codebase
- 🔧 **Edge Case Testing Needed** - Test with poor network conditions, VPN, corporate proxy to validate retry logic

## Context for Future
This fix establishes the pattern for "pre-warm on launch, use on demand" that could be applied to other startup optimizations (e.g., pre-loading user settings, pre-fetching permissions state). The `retryWithBackoff()` utility in `src/utils/retryWithBackoff.ts` is now available as a reusable pattern for any network-dependent operations - just import it and provide the function + context string. Future work on reducing app launch latency should preserve the sequential JWT→PreConnect ordering, as breaking this sequence would reintroduce the auth latency on first dictation.

The AUTH.md documentation (lines 686-745) already documented the "Pre-Connect Pattern" philosophy - this session implemented it correctly with proper sequencing and retry logic that was missing from the original implementation. The DRY refactoring into a utility function makes this pattern easy to apply elsewhere in the codebase.
