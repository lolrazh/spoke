...
export function logClientSession(event: ClientSessionEvent): void {
    ...
    const level = event.outcome === 'success' || event.outcome === 'cancelled' ? 'info' : 'error';
 ...
 * Usage:
 * 
 * const builder = new ClientSessionEventBuilder(sessionId, mode);
 * builder.setPttDown(performance.now());
 * // ... session progresses ...
 * builder.setOutcome('success', finalText);
...
export class ClientSessionEventBuilder {
    ...
    constructor(sessionId: string, mode: 'dictation' | 'edit') {
        ...
        this.event = {
            ...
            trace_id: sessionId, // Will be set from server if available
    ...
    emit(): void {
        if (!this.event.outcome) {
            console.warn('[ClientSession] emit() called without outcome - setting to error_unknown');
