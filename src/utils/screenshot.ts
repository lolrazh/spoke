...
export async function captureScreenshot(
  ...
  const { display = "active", quality = 75, maxDimension = 1080 } = options;
  ...
  const source = sources.find((s) => {
    ...
    return (
      s.name.includes(targetDisplay.id.toString()) ||
  ...
  if (!source) {
    throw new Error(
      `Could not find screen source for display ${targetDisplay.id}`,
    );
  }
