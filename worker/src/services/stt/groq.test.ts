...
import { transcribeWav } from "./providers/groq";
...
describe("services/stt/groq.transcribeWav", () => {
  ...
  it("posts wav to groq and returns text + timings", async () => {
    ...
    expect(result.timings.bodyDoneAt).toBeGreaterThanOrEqual(
      result.timings.headersAt,
    );
  ...
  it("propagates non-2xx as error", async () => {
    ...
    await expect(
      transcribeWav(wav, apiKey, { timeoutMs: 1000 }),
    ).rejects.toThrow(/GROQ STT error/);
  });
  ...
  it("aborts on timeout", async () => {
    ...
    await expect(
      transcribeWav(wav, apiKey, { timeoutMs: 1 }),
    ).rejects.toThrow();
  });
});
