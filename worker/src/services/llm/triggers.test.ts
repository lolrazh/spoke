...
describe('services/llm/triggers', () => {
  describe('detectTriggers - spelling', () => {
    ...
    it('detects "spelled" and "spelling" variants', () => {
      ...
      expect(result1.metadata.spelling?.instructions[0].keyword).toBe('spelled');

      const result2 = detectTriggers('The spelling is incorrect');
      expect(result2.triggers.has('spelling')).toBe(true);
      expect(result2.metadata.spelling?.instructions[0].keyword).toBe('spelling');
    });

    it('does not trigger on normal text', () => {
      const result = detectTriggers('This is a normal sentence with no special instructions');
  ...
  describe('detectTriggers - disfluency', () => {
    ...
    it('detects wait no', () => {
      ...
      expect(result.triggers.has('disfluency')).toBe(true);
      expect(result.metadata.disfluency?.corrections[0].keyword).toMatch(/wait no/i);
    });

    it('detects scratch that', () => {
      const result = detectTriggers('Send it to John scratch that send it to Jane');
      expect(result.triggers.has('disfluency')).toBe(true);
    });
    ...
    it('detects actually', () => {
      const result = detectTriggers('Set the color to blue actually make it red');
      expect(result.triggers.has('disfluency')).toBe(true);
    });
  ...
  describe('detectTriggers - lists (numeric)', () => {
    it('detects numeric lists with 3+ items', () => {
      const result = detectTriggers('First item is 1 buy milk, 2 walk the dog, 3 finish report');
    ...
    it('detects longer numeric lists', () => {
      const result = detectTriggers('Tasks: 1 email, 2 call, 3 meeting, 4 report, 5 review');
      expect(result.triggers.has('list')).toBe(true);
    ...
    it('extracts item text correctly', () => {
      const result = detectTriggers('List: 1 buy groceries, 2 clean house, 3 do laundry');
      expect(result.triggers.has('list')).toBe(true);
  ...
  describe('detectTriggers - lists (ordinal)', () => {
    it('detects ordinal lists', () => {
      const result = detectTriggers('Steps: first turn on computer, second open browser, third navigate to site');
      expect(result.triggers.has('list')).toBe(true);
    ...
    it('requires consecutive ordinals', () => {
      const result = detectTriggers('Items: first one, third three, fourth four');
      expect(result.triggers.has('list')).toBe(false);
    });

    it('detects longer ordinal lists', () => {
      const result = detectTriggers('First task, second task, third task, fourth task, fifth task');
      expect(result.triggers.has('list')).toBe(true);
      expect(result.metadata.list?.items).toHaveLength(5);
    });
  });
  ...
  describe('detectTriggers - lists (alphabetic)', () => {
    it('detects alphabetic lists', () => {
      const result = detectTriggers('Options: a choice one, b choice two, c choice three');
      expect(result.triggers.has('list')).toBe(true);
  ...
  describe('detectTriggers - multiple triggers', () => {
    it('detects multiple triggers in same text', () => {
      const result = detectTriggers('Make it uppercase and add an at symbol, also spell C L A U D E');
      expect(result.triggers.has('casing')).toBe(true);
      expect(result.triggers.has('symbols')).toBe(true);
      expect(result.triggers.has('spelling')).toBe(true);
    ...
    it('detects list with corrections', () => {
      const result = detectTriggers('first buy milk, second walk dog, third clean house, sorry actually first is buy bread');
      expect(result.triggers.has('list')).toBe(true);
      expect(result.triggers.has('disfluency')).toBe(true);
    });
  });
  ...
  describe('detectTriggers - edge cases', () => {
    ...
    it('handles clean dictation with no triggers', () => {
      const result = detectTriggers('This is a perfectly normal sentence with no special instructions whatsoever.');
      expect(result.triggers.size).toBe(0);
      expect(result.requiresLLM).toBe(false);
    });
