...
import { composeDynamicPrompt, estimatePromptTokens, getPromptStats } from './composer';
import { detectTriggers } from '../triggers';

describe('services/llm/prompts/composer', () => {
  describe('composeDynamicPrompt', () => {
    it('includes only base prompt for clean dictation', () => {
      ...
      expect(prompt).not.toContain('spelling_rules');
      expect(prompt).not.toContain('symbol_rules');
      expect(prompt).not.toContain('correction_rules');
    ...
    it('includes spelling module when spelling trigger fires', () => {
      ...
      expect(prompt).toContain('You are a verbatim ASR cleaner');
      expect(prompt).toContain('<spelling_rules>');
    ...
    it('includes symbols module when symbol trigger fires', () => {
      ...
      expect(prompt).toContain('<symbol_rules>');
      expect(prompt).toContain('<examples>');
    ...
    it('includes casing module when casing trigger fires', () => {
      ...
      expect(prompt).toContain('<casing_rules>');
      expect(prompt).toContain('<examples>');
      expect(prompt).not.toContain('symbol_rules');
    });
    ...
    it('includes quotes module when quote trigger fires', () => {
      ...
      expect(prompt).toContain('<quote_rules>');
      expect(prompt).toContain('<examples>');
      expect(prompt).toContain('quote-unquote');
    });

    it('includes corrections module when disfluency trigger fires', () => {
      const triggerContext = detectTriggers('Turn left sorry turn right');
      const prompt = composeDynamicPrompt(triggerContext);

      expect(prompt).toContain('<correction_rules>');
      expect(prompt).toContain('<examples>');
    ...
    it('includes lists module when list trigger fires', () => {
      ...
      expect(prompt).toContain('<list_rules>');
      expect(prompt).toContain('<examples>');
  ...
  describe('token savings comparison', () => {
    ...
    it('saves ~60-70% tokens for single trigger vs all triggers', () => {
      ...
      // Should save at least 50% tokens
