...
export function logSessionOCR(data: SessionOCRLog): void {
  ...
  const level: LogLevel = outcome === 'success' ? 'info' : outcome === 'error' ? 'error' : 'warn';

  let message: string;
  if (outcome === 'success' && word_count !== undefined && duration_ms !== undefined) {
...
export function logSessionSTT(data: SessionSTTLog): void {
  const { outcome, provider, model, duration_ms, ttfb_ms, text_length, trace_id, error_message } = data;
...
export function logSessionLLM(data: SessionLLMLog): void {
  const { outcome, provider, model, duration_ms, ttfb_ms, router_overhead_ms, text_length, trace_id, error_message } = data;

  const level: LogLevel = outcome === 'success' ? 'info' : outcome === 'skipped' ? 'debug' : 'error';
