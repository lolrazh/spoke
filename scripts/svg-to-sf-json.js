...
function parseArgs(argv) {
  ...
  if (positional.length < 2) {
    console.error('Usage: node scripts/svg-to-sf-json.js <svgPath> <iconName> [--weight bold] [--json public/assets/sf-symbols.json] [--merge]');
    process.exit(1);
  }
...
function extractViewBox(svg) {
  const viewBoxMatch = svg.match(/viewBox=["']([\d.+\-eE]+)\s+([\d.+\-eE]+)\s+([\d.+\-eE]+)\s+([\d.+\-eE]+)["']/);
...
function main() {
  const { svgPath, iconName, weight, jsonPath, merge } = parseArgs(process.argv);
  const svg = fs.readFileSync(svgPath, 'utf8');

  const geometry = extractViewBox(svg);
  if (!geometry) {
    console.error('Could not parse viewBox from SVG. Ensure it contains viewBox="minX minY width height".');
    process.exit(2);
  }

  const d = extractPathD(svg);
  if (!d) {
    console.error('Could not find a <path d="..."> element in the SVG. Multiple paths are not supported by this simple converter.');
    process.exit(3);
  }
